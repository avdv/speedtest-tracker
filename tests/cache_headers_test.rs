...
async fn test_css_cache_headers() {
    let response = embedded_assets::serve_css(axum::extract::Path("rust-app.css".to_string())).await;
    ...
    assert!(cache_value.contains("public"), "Should be publicly cacheable");
    assert!(cache_value.contains("max-age=31536000"), "Should cache for 1 year");
    assert!(cache_value.contains("immutable"), "Should be marked as immutable");
...
async fn test_favicon_cache_headers() {
    ...
    // Check Cache-Control
    let cache_control = response.headers().get("cache-control");
    assert!(cache_control.is_some(), "Cache-Control header should be present for favicon");
    ...
    assert!(ct == "image/x-icon" || ct == "image/vnd.microsoft.icon", 
            "Content-Type should be a valid icon MIME type, got: {}", ct);
}

#[tokio::test]
async fn test_nonexistent_file_no_cache_headers() {
    let response = embedded_assets::serve_css(axum::extract::Path("nonexistent.css".to_string())).await;
